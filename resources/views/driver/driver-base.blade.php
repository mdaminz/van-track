...
<body>
                        ...
                        <ul class="navbar-nav flex-column mb-3" id="navbarVerticalNav">
                            ...
                            <ul class="nav collapse false" id="report">
                                <li class="nav-item"><a class="nav-link" href="/schedule_list">
                                        <div class="d-flex align-items-center"><span class="nav-link-text ps-1">RT
                                                Attendance</span>
                                        </div>
                                    </a>
                                    <!-- more inner pages-->
                            ...
                            <li class="nav-item">
                                <!-- label-->
                                <div class="row navbar-vertical-label-wrapper mt-3 mb-2">
                                    <div class="col-auto navbar-vertical-label">Manage Report
                                    </div>
                                    <div class="col ps-0">
                                        <hr class="mb-0 navbar-vertical-divider" />
                                    </div>
                        ...
                        <li class="nav-item">
                            <h5 class="text-primary d-none d-md-block"
                                ...
                                <small class="badge fw-semi-bold rounded-pill status badge-soft-primary"
                                    style="margin-right: 10px">Driver</small>
                                {{ Auth::user()->name }}
    ...
    <div class="container mt-4">
        <div style="hidden" id="map" style="height: 500px; width: 100%;"></div>
    </div>
    ...
    <script>
        ...
        function initMap() {
            map = new google.maps.Map(document.getElementById("map"), {
                center: { lat: 0, lng: 0 }, // Default position
                zoom: 15,
            });

            if (navigator.geolocation) {
                // Get the current location every second
                setInterval(() => {
                    navigator.geolocation.getCurrentPosition(position => {
                        const lat = position.coords.latitude;
                        const lng = position.coords.longitude;

                        // Send the coordinates to the backend to update the database
                        fetch("{{ route('update.coor') }}", {
                            method: "POST",
                            headers: {
                                "Content-Type": "application/json",
                                "X-CSRF-TOKEN": document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                            },
                            body: JSON.stringify({ coor: `${lat},${lng}` })
                        });
                        ...
                        if (!marker) {
                            ...
                            marker.setPosition(pos); // Update position of the marker
                        }
